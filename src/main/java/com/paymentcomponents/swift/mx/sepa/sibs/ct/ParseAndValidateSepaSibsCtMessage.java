package com.paymentcomponents.swift.mx.sepa.sibs.ct;

import gr.datamation.iso20022.sepa.sibs.ct.headers.CvfBulkCreditTransferSepaSibsCt;
import gr.datamation.iso20022.sepa.sibs.ct.pacs.FIToFIPaymentStatusReport10SepaSibsCt;
import gr.datamation.validation.error.ValidationError;
import gr.datamation.validation.error.ValidationErrorList;

import java.io.ByteArrayInputStream;

public class ParseAndValidateSepaSibsCtMessage {

    public static void main(String... args) {
        execute();
    }
    public static void execute() {
        constructSepaSibsCtMessage();
        parseAndValidateSepaSibsCtMessage();
        constructSepaSibsCtHeader();
        parseAndValidateSepaSibsCtHeader();
    }

    public static void constructSepaSibsCtMessage() {
        try {
...
//            fIToFIPaymentStatusReport10.getMessage().getGrpHdr().setMsgId("1234");
    ...
    public static void parseAndValidateSepaSibsCtMessage() {
        try {
            //Initialize the message object
            FIToFIPaymentStatusReport10SepaSibsCt fIToFIPaymentStatusReport10 = new FIToFIPaymentStatusReport10SepaSibsCt();
    ...
    public static void constructSepaSibsCtHeader() {
        try {
            //Initialize the message object
            CvfBulkCreditTransferSepaSibsCt cvfHeader = new CvfBulkCreditTransferSepaSibsCt();
            //Optionally use an existing message if we do not want to create the object from scratch
            cvfHeader.parseXML(validSepaSibsCtCvfHeaderString);
...
//            In case we already have a message object, we can set it to the header
    ...
    public static void parseAndValidateSepaSibsCtHeader() {
        try {
            //Initialize the message object
            CvfBulkCreditTransferSepaSibsCt cvfHeader = new CvfBulkCreditTransferSepaSibsCt();
            //Validate against the xml schema
            ValidationErrorList validationErrorList = cvfHeader.validateXML(new ByteArrayInputStream(validSepaSibsCtCvfHeaderString.getBytes()));
            if (!validationErrorList.isEmpty()) {
                handleValidationError(validationErrorList);
                return;
            }
    ...
    private static final String validSepaSibsCtPacs002String = "<Document xmlns=\"urn:iso:std:iso:20022:tech:xsd:pacs.002.001.10S2\">\n" +
        "    <FIToFIPmtStsRptS2>\n" +
        "        <GrpHdr>\n" +
    ...
    private static final String validSepaSibsCtCvfHeaderString = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
        ...
        "                           xsi:schemaLocation=\"urn:S2SCTCvf:xsd:$SCTCvfBlkCredTrf SCTCvxBlkCredTrf.xsd\"\n" +
        ">\n" +
        ...
        "    <S2SCTCvf:FType>CVX</S2SCTCvf:FType>\n" +
        "    <S2SCTCvf:FileRef>2018111800000001</S2SCTCvf:FileRef>\n" +
        "    <S2SCTCvf:ClrSys>str</S2SCTCvf:ClrSys>\n" +
